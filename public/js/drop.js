...
function modalDrop() {
  var html =
    ...
    "</div>" +
    "</div>" +
    '<label style="margin: -1% auto; min-width: 60%; color: #f6b504 !important; cursor:pointer" class="label"><a href="http://localhost:6060/modelo.xlsx">Download planilha sem produtos</a></label>' +
    "</div>" +
    '<div   class="input-group btnDrop">' +
    '<div style="border: none; margin: auto;" class="input-group-append">' +
    '<div style="border: none; margin: auto; padding: 1%;" class="input-group-text">' +
    ...
    onShow: function () {
      ...
      $("#minhaPlanilha").change(function () {
        try {
          ...
          $.ajax({
            url: "http://localhost:6060/uploadPlanilha",
            headers: {
            ...
            success: function (data) {
              ...
              $.ajax({
                url: "http://localhost:6060/excel2Json",
                headers: {
                  "x-access-token": localStorage.token,
                  master_id: localStorage.MASTER_ID,
                  affiliate_id: localStorage.MASTER_ID,
...
function finalizaAjustes(listOffQuerys) {
  ...
  $.ajax({
    type: "POST",
    url: "http://localhost:6060/multiQuerys",
